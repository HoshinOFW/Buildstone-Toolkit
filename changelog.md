...
- Added schematic support. If a proxy's target is within the schematic's bounding box, its position will be saved and loaded via a relative offset.
  ...
  - Proxy targets follow contraption assembly/disassembly. This means you can "move" a target using create contraptions.
    - Even unloaded proxies!
...
- Proxies can now target into and out of sublevels. 
  - Proxy targets (loaded or unloaded) also properly move into the sublevels on assembly.
    - Even unloaded proxies!
  - If you don't care for the technical jargon, this essentially means that most things you can imagine should work now work.

### Features:
- Redstone proxies have been upgraded. They now have two modes: 
  - Read and Write (You can switch between them by smacking the proxy with the tuner). 
  - Additionally, they can target a specific face instead of the whole block, allowing more precise control.
...
- Proxy targets can be stored via relative offset in NBT. This means structures/worldgen can properly use them now!

### Bugs/Performance
- Made mixins less invasive by using modern MixinExtras features, improving compatibility with other mods.
- Improved performance overall for all systems (redstone, rendering, etc.)

### Old worlds
- Won't break, and migration is automatic as you load in the proxies.